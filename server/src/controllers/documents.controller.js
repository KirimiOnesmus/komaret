...
export const create = asyncHandler(async (req, res) => res.status(201).json(await service.create(req.file, req.body, req.user)));
export const download = asyncHandler(async (req, res) => {
  const { stream, filename, mimeType } = await service.getFileStream(req.params.id, req.user);
