...
export default function NewsImage({ articleId, image, onUpload, onRemove }) {
  ...
  const onPick = (e) => {
    ...
    e.target.value = '';
    if (!file) return;
    run(() => onUpload(articleId, file));
  };

  const handleRemove = () => {
    if (!window.confirm('Remove this image?')) return;
    run(() => onRemove(articleId));
  };

  return (
    <div>
      <h2 className="text-sm font-bold text-[#071525]">Cover image</h2>
      <p className="mt-0.5 text-xs text-gray-400">
        Shown on the news listing and at the top of the article. PNG, JPEG or WebP.
      </p>
      ...
      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end">
        <div className="relative aspect-[16/9] w-full max-w-xs overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
          {image ? (
            <img src={mediaUrl(image)} alt="Article cover" className="h-full w-full object-cover" />
          ) : (
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            <FaUpload className="text-xs" /> {image ? 'Replace image' : 'Upload image'}
          </button>
          {image && (
            <button
              type="button"
